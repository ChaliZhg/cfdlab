...
void ScalarProblem<dim>::make_grid_and_dofs (unsigned int step)
{
   ...
   average.resize (triangulation.n_cells(), Vector<double>(fe.dofs_per_cell));
...
void ScalarProblem<dim>::initialize ()
{
   ...
   QGauss<dim>  quadrature_formula(2 * fe.get_degree() + 1);
   ...
   for (unsigned int c=0; cell!=endc; ++cell, ++c)
   {
      ...
      for (unsigned int i=0; i<dofs_per_cell; ++i)
      {
         ...
         for (unsigned int j=0; j<dofs_per_cell; ++j)
            solution (ig) += cell_rhs(j) * inv_mass_matrix[c](i,j);
      }
   }
}
...
void ScalarProblem<dim>::assemble_mass_matrix ()
{
    ...
    std::cout << "  Quadrature using " << fe.get_degree()+1 << " points\n";

    QGauss<dim>  quadrature_formula(fe.get_degree()+1);
    ...
    for (unsigned int c=0; cell!=endc; ++cell, ++c)
    {
       ...
       numerical_flux (flux_type, rf_left_state, rf_right_state, 
        ...
        for (unsigned int i=0; i<dofs_per_cell; ++i)
        {
            ...
            rhs(ig) = 0;
            for (unsigned int j=0; j<dofs_per_cell; ++j)
               rhs (ig) += cell_rhs(j) * inv_mass_matrix[c](i,j);
...
void ScalarProblem<dim>::mark_troubled_cells ()
{
   ...
   for (unsigned int c=0; c<n_cells; ++c, ++cell)
   {
      ...
      // get neighbouring cell by periodicity
      unsigned int cl, cr;
      if(c==0)
      {
         cl = n_cells-1;
...
void ScalarProblem<dim>::apply_weno_limiter ()
{
   ...
   FEValues<dim> fe_values_left (fe, quadrature_formula,
                            update_values | update_quadrature_points);
   FEValues<dim> fe_values_right (fe, quadrature_formula,
                                  update_values | update_quadrature_points);
   const unsigned int   dofs_per_cell = fe.dofs_per_cell;
   const unsigned int   n_q_points    = quadrature_formula.size();
   std::vector<double> sol_left (n_q_points), sol_right(n_q_points);
   ...
   for (unsigned int c=0; cell!=endc; ++c, ++cell)
   {
      ...
      fe_values_left.reinit (lcell[c], fe_values.get_quadrature_points());
      fe_values_right.reinit (rcell[c], fe_values.get_quadrature_points());
      
      fe_values_left.get_function_values (solution, sol_left);
      fe_values_right.get_function_values (solution, sol_right);
      
      // average of extended solution on current cell
      double avg_left = 0, avg_right = 0;
      for(unsigned int q=0; q<n_q_points; ++q)
      {
         avg_left += sol_left[q] * fe_values.JxW(q);
         avg_right += sol_right[q] * fe_values.JxW(q);
      }
      avg_left /= cell->measure();
      avg_right /= cell->measure();
...
{
   if (fe.get_degree() == 0 || limiter_type == none) return;
...
void ScalarProblem<dim>::output_results (const double& time) const
{
   static unsigned int c = 0;
    ...
    if(fe.get_degree() <= 1)
       ...
       data_out.build_patches (2*fe.get_degree());

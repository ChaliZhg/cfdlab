subroutine numflux_z(conjm1, conj, conjp1, conjp2, flux)
   use comvar
   implicit none
   ...
   if(fluxtype == ikep)then
      call kep_flux(0.0, 0.0, 1.0, conl, conr, flux)
   else if(fluxtype == ikepes)then
      call kepes_flux(0.0, 0.0, 1.0, conl, conr, flux)
   else
   ...
   if(ikepes_diss == yes)then
      call kepes_diss(0.0, 0.0, 1.0, conjm1, conj, conjp1, conjp2, dflux)
      flux = flux - dflux
   endif

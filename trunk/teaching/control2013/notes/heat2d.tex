...
\[
 \dd{}{t}(z(t), \phi)_{L^2} = - \mu \int_\Omega \nabla z \cdot \nabla \phi \ud x +  \omega \int_\Omega z \phi \ud x, \quad \forall \phi \in H^1_{\Gamma_D}(\Omega)
\]
\[
...
\[
 \dd{}{t}(z(t), \phi_i)_{L^2} = - \mu \int_\Omega \nabla z \cdot \nabla \phi_i \ud x +  \omega \int_\Omega z \phi_i \ud x, \quad \forall i \in N_f
\]
i.e.,
\begin{eqnarray*}
...
\[
 \M \dd{\z}{t} = \A \z + \B v
\]
where $\z$ denotes all the unknown degrees of freedom.
%-----------------------------------------------------------------
...
\vspace{2mm}\noindent
Jochen Alberty, Carsten Carstensen and Stefan A. Funken: ``Remarks around 50 lines of Matlab: short finite element implementation", Numerical Algorithms 20 (1999) 117Ð-137\\
...
\item Generate the mesh by running the {\tt square} program and use 81 points on each side of the square.

\item The value of $\omega$ is set in the file {\tt parameters.m}. Set $\omega=0$ and calculate five eigenvalues with largest real part using {\tt eigs} function. Is there an  unstable eigenvalue ?

\item With $\omega=0$, run the program. Observe that the energy is decreasing with time. The energy is plotted with linear scale for time and log scale for energy. The straight line behaviour indicates that the energy is decaying exponentially wrt time.

\item With $\omega=0$, find the decay rate of the solution.

\item Set $\omega=0.4$ and caculate five eigenvalues with largest real part using {\tt eigs} function. Is there an unstable eigenvalue ? Compare it with exact unstable eigenvalue given above.
...
The feedback control is determined by solving the lqr problem using the matrices $\M$, $\A$ and $\B$. Since the dimensions of these matrices are very large, we determine the feedback matrix $\K$ by considering only the unstable components of the system.
%-----------------------------------------------------------------
\section{Excercises II}
\begin{enumerate}

\item Set $\omega = 0.4$. In {\tt fem\_50.m}, insert the code to determine the the feedback matrix $K$ using only the unstable components (in this case, there will be just one unstable component). Refer back to the codes for the 1D heat problem if needed. Remember to modify the matrix $\A1$ in the code accordingly. Run the code.

\item Save the matrix $\K$ obtained above. Set $\omega=0$ and load $\K$ into the code instead of evaluating it. Run the code and observe the decay rate of the solution. Is it as expected?
...
%-----------------------------------------------------------------
\section{Excercises III}
\begin{enumerate}

\item Set $\omega = 0.4$. In {\tt fem\_50\_est.m}, insert lines of code to determine $\K$ as above, and to evaluate $\bL$ using just the unstable components. You will need the matrix $\bH$ which is already generated by {\tt get\_matrices.m}. Run the code and observe the solution.

\item Save the matrix $\K$ and $\bL$ obtained above. Set $\omega=0$ and load these matrices into the code instead of evaluating them. Run the code and evaluate the decay rate of the solution. Is it as expected?
...
The programs are under the directory {\tt heat\_2d}.

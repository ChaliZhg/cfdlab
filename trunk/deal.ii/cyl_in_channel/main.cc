...
void declare_parameters(ParameterHandler& prm)
{
   ...
   prm.declare_entry("mesh file", "cyl.msh",
                     Patterns::Anything(),
                     "Mesh file name");
                            ...
                            const unsigned int component) const
{
   ...
   if (component == 0)
      return (1.5/std::pow(0.2,2))*(0.2+p[1])*(0.2-p[1]);
   else
      return 0.0;
}
...
void NS<dim>::assemble_matrix_and_rhs (unsigned int order)
{
   ...
   // Compute residual to monitor convergence to steady state
   // We must do this before applying boundary conditions.
   if(run_mode == steady)
   {
      Vector<double> tmp(dof_handler.n_dofs());
      system_matrix.vmult (tmp, solution2);
      std::map<types::global_dof_index,double>::iterator it = boundary_values.begin();
      for(; it != boundary_values.end(); ++it)
         tmp(it->first) = 0.0;
      std::cout << "Residual = " << tmp.l2_norm() / sqrt(dof_handler.n_dofs()) << std::endl;
   }
...
{
   std::cout << "Performing steady state computation ...\n";

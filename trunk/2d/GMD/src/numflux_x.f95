subroutine numflux_x(conjm1, conj, conjp1, conjp2, flux, dflux)
   use comvar
   implicit none
   ...
   if(fluxtype == iroe)then
      call roe_flux(1.0, 0.0, conl, conr, flux, dflux)
   else if(fluxtype == irusanov)then
      call rusanov_flux(1.0, 0.0, conl, conr, flux, dflux)
   else if(fluxtype == iadv)then
      call adv_flux(1.0, 0.0, conl, conr, flux, dflux)
   else if(fluxtype == icusp)then
      call cusp_flux(1.0, 0.0, conl, conr, flux, dflux)
   else if(fluxtype == ikep)then
      call kep_flux(1.0, 0.0, conl, conr, flux, dflux)
   else if(fluxtype == ikepes)then
      call kepes_flux(1.0, 0.0, conl, conr, flux, dflux)
   else
   ...
   if(ikepes_diss == yes)then
      call kepes_diss(1.0, 0.0, conjm1, conj, conjp1, conjp2, dflux)
      flux = flux - dflux
   endif

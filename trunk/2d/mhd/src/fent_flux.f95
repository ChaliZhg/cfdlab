...
!------------------------------------------------------------------------------
subroutine fent_flux(lx, ly, ll, left, right, rr, flux)
   use comvar
   implicit none

   real :: lx, ly
   real :: left(nvar), right(nvar), ll(nvar), rr(nvar), flux(nvar)
   ...
   real :: Diff
   real :: zll(nvar), zl(nvar), zr(nvar), zrr(nvar), dz(nvar), zleft(nvar), &
           zright(nvar)
   integer :: i, j, k
   ...
   ! Compute z
   call computez(R, ll,    zll)
   call computez(R, left,  zl)
   call computez(R, right, zr)
   call computez(R, rr,    zrr)
   call reconstruct(zll, zl, zr, zrr, zleft, zright)

   dz = Lambda * (zright - zleft)
   
   do i=1,nvar
      Diff = 0.0
      do j=1,nvar
         Diff = Diff + R(i,j) * dz(j)
      enddo
...
!----------------------------------------------------------------------------
subroutine computez(R, prim, z)
   use comvar
   implicit none

   real :: R(nvar,nvar), prim(nvar), z(nvar)

   real :: s, beta, q2, q(nvar)
   integer :: i, j

   ! Compute entropy variable
   s    = log(prim(5)) - GAMMA * log(prim(1))
   beta = 0.5*prim(1)/prim(5)
   q2   = prim(2)**2 + prim(3)**2 + prim(4)**2

   q(1)   =-s/(GAMMA-1.0) - beta*q2
   q(2:4) = 2.0*beta*prim(2:4)
   q(5)   =-2.0*beta
   q(6:8) = 2.0*beta*prim(6:8)
